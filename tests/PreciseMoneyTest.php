...
final class PreciseMoneyTest extends \PHPUnit_Framework_TestCase
{
    use RoundExamples;
    ...
    public function it_equals_to_another_money($amount, $currency, $equality)
    {
        ...
        $this->assertEquals($equality, $money->equals(new PreciseMoney($amount, $currency)));
    }
    ...
    {
        $money = new PreciseMoney(self::AMOUNT, new Currency(self::CURRENCY));
        $other = new PreciseMoney($other, new Currency(self::CURRENCY));
    ...
    {
        $money = new PreciseMoney($amount, new Currency(self::CURRENCY));

        $allocated = $money->allocate($ratios);

        foreach ($allocated as $key => $money) {
            $compareTo = new PreciseMoney($results[$key], $money->getCurrency());
    ...
    public function test_it_converts_to_json()
    {
        $this->assertEquals(
            ...
            json_encode(PreciseMoney::EUR(350))
        );
    }
    ...
    public function test_it_supports_max_int()
    {
        ...
        $this->assertInstanceOf(PreciseMoney::class, new PreciseMoney(PHP_INT_MAX, new Currency('EUR')));
        $this->assertInstanceOf(PreciseMoney::class, (new PreciseMoney(PHP_INT_MAX, new Currency('EUR')))->add($one));
