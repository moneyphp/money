<?php

/**
 * This file is part of the Money library.
 *
 * Copyright (c) 2011-2014 Mathias Verraes
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */

namespace Money;

/**
 * @coversDefaultClass Money\Currencies
 * @uses Money\Currency
 * @uses Money\Currencies
 */
class CurrenciesTest extends \PHPUnit_Framework_TestCase
{
    public function setUp()
    {
        $this->currencies = new Currencies(array(
            "EUR" => "Euro",
            "USD" => "United States Dollar",
        ));
    }

    /**
     * @covers ::__construct
     */
    public function testConstructor()
    {
        $currencies = new Currencies(array(
            "EUR" => "Euro",
            "USD" => "United States Dollar",
        ));

        $this->assertTrue($currencies->exists('EUR'));
        $this->assertTrue($currencies->exists('USD'));
        $this->assertFalse($currencies->exists('ASD'));
    }

    /**
     * @covers ::exists
     */
    public function testExists()
    {
        $this->assertTrue($this->currencies->exists('EUR'));
        $this->assertTrue($this->currencies->exists('USD'));
        $this->assertFalse($this->currencies->exists('ASD'));
    }

    /**
     * @covers ::exists
     * @covers ::assertExists
     * @expectedException Money\UnknownCurrencyException
     */
    public function testAssertExistsFailed()
    {
        $this->currencies->assertExists('ASD');
    }

    /**
     * @covers ::createCurrency
     */
    public function testCreateCurrency()
    {
        $currency = $this->currencies->createCurrency('EUR');

        $this->assertInstanceOf('Money\\Currency', $currency);
        $this->assertEquals('EUR', $currency->getCode());
    }
}
